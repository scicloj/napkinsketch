...
**napkinsketch** — composable plotting in Clojure

Four-stage pipeline: views → sketch → membrane → figure. See README.md for details.
...
Tests include:
- Hand-written tests in `test/scicloj/napkinsketch/`
- Generated tests from notebooks (via `kind/test-last`) in `test/napkinsketch_book/`
...
- `test/scicloj/napkinsketch/` — hand-written tests
- `test/napkinsketch_book/*_generated_test.clj` — generated from notebooks via Clay GFM rendering

### Generated Test Workflow

Notebooks use `kind/test-last` to embed assertions. When rendered to GFM via Clay,
test files are generated in `test/napkinsketch_book/`. These are run by the standard test runner.

To regenerate tests after editing a notebook:
```clojure
(require '[dev :as dev] :reload)
(dev/make-gfm! "napkinsketch_book/<name>.clj")
```
...
- **notebooks/** — Clay-based documentation
  ...
  - `napkinsketch_book/` — Chapter notebooks
- **src/** — Source code
...
- **No abbreviations in prose**: Use full words in notebook prose (e.g., "options" not "opts", "configuration" not "config"). Abbreviations in invisible test code (`kind/test-last`) are fine.
- **No private vars in notebooks**: Notebooks are teaching material — all defs should be public. Use `def` not `def ^:private` or `defn-`. Both `defn-` and `def ^:private` display var names in rendered output; just use plain `def`.
- **No single-use wrapper functions in notebooks**: If a function is called only once for its output, inline its body directly instead of wrapping it in a `defn`. The notebook reader sees the logic immediately.
...
- **No unnecessary `tc/dataset`**: When data is a plain Clojure map or vector of row maps passed to `sk/view` or mark constructors, do not wrap it in `tc/dataset` — the coercion happens internally. Only use `tc/dataset` when loading from a URL/file (where `{:key-fn keyword}` is needed) or when the result is used with `tc/` operations (e.g., `tc/select-rows`).
- **Thread sketches into pipelines**: Write `(-> data (sk/lay-point :x :y) (sk/options {:title "..."}))` as a single `->` pipeline. Every function takes a sketch and returns a sketch.
- **Use `kind/pprint` sparingly**: Most values render well in notebooks without wrapping. Only use `kind/pprint` for: (1) napkinsketch **views** — they contain a tc dataset that would otherwise render as an elaborate nested structure; (2) final **figures** (SVG hiccup) when you want to inspect the data structure instead of seeing the rendered plot. Plain maps, sketches, and other values do not need it.
- **Test all numerical prose**: Every specific number in `;;` comments (counts, dimensions, RGBA values, percentages, domain bounds) must be backed by a `kind/test-last` check. Use exact values (`= 100`, `== 0.8`) not loose checks (`pos?`, `< 1.0`) when the prose states an exact number.
...
## Sketch/Plan Naming Convention

The pipeline has two user-visible types — **sketch** and **plan** — that are easy to confuse.

| Type | Created by | Variable names | Prose term |
|:-----|:-----------|:---------------|:-----------|
| Sketch | `sk/view`, `sk/lay-*`, `sk/options`, `sk/facet`, `sk/coord`, `sk/scale` | `my-sketch`, `scatter-base`, `iris-sketch` | "sketch" |
| Plan | `sk/plan` | `my-plan`, `good-plan`, `trace-plan` | "plan" |

**Rules:**
- Never name a `sk/plan` result `*-sketch`. Never name a `sk/lay-*` result `*-plan`.
- In test fns: `(fn [plan] ...)` after `sk/plan`, `(fn [v] ...)` or `(fn [sketch] ...)` after sketch-producing calls.
- In prose: say "plan" when discussing `sk/plan` output, domains, panels, layers, ticks. Say "sketch" when discussing composable user-facing values.
- Run `dev-tools/verify-terminology.sh` after any notebook edit to catch violations.
...
### Coercion guidelines: `vec`, `seq`, `mapv`

**Structural vs numeric collections.** `(vec (for ...))` is correct for building structural collections — vectors of maps, drawing primitives, legend entries, panel descriptors. These are Clojure data structures, not numeric arrays. Never remove `vec` from these.

**`vec` on numeric data leaving the pipeline.** Use `vec` only when the consumer requires an `IPersistentVector` (e.g., a Malli schema with `[:vector ...]`). If the schema accepts `[:sequential ...]`, dtype buffers and lazy seqs pass through — no materialization needed.

**`seq` as a function argument is almost never needed.** `map`, `reduce`, `mapv`, `sort`, `group-by`, `stats/stddev`, and most Clojure core functions accept arrays, Columns, and dtype buffers directly. Drop `(seq x)` wrappers unless the function specifically requires an ISeq.

**`(vec (seq x))` is always wrong** — double coercion. Either `vec` alone (to materialize) or nothing (if materialization is unnecessary).

**`(vec (map f coll))` → `(mapv f coll)`** — `mapv` is strictly better: one allocation instead of two (lazy seq + vec copy).

**`mapv f buf` vs `dtype/emap f :float64 buf`** — When the input is a dtype buffer and the result should stay in dtype-land (fed to `dfn` ops or stored in sketch), use `dtype/emap`. When the result is a structural collection of non-numeric values (maps, strings), `mapv` is correct.

**`(vec nil)` masks bugs.** `(vec nil)` returns `[]`, silently hiding nil-propagation errors. Prefer explicit nil guards: `(if coll (.indexOf ^java.util.List coll val) -1)` rather than `(.indexOf ^java.util.List (vec coll) val)`.

**Decision tree for sketch boundary data:**

```
Is it a numeric array (xs, ys, sizes, alphas, densities)?
  → Leave as dtype buffer. Schema uses [:sequential number?].
Is it a collection of structural maps (groups, boxes, tiles, legend entries)?
  → Use (vec (for ...)) or (mapv ...). Schema uses [:vector ...].
Is it labels/categories (strings/keywords)?
  → Use (mapv str ...) or (vec (distinct ...)). Schema uses [:vector string?].
```
...
## Code Style: Prefer defn over macros

Avoid macros for API function definitions unless there is a compelling reason.
Use regular `defn` even if it means code repetition across similar functions.

**Why:** Macros hurt tooling — IDE navigation, docstring lookup, arglists
display, and REPL introspection all work better with plain `defn`. Generated
functions are opaque to tools like cider-doc, clj-kondo, and LSP.

**How:** Use shared private helper functions to avoid logic duplication, but
keep the public `defn` explicit. For example, the 25 `lay-*` functions should
each be a `defn` that calls common helpers (`add-entry-method`,
`build-method-map`), not a macro expansion.
