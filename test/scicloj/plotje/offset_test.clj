...
(defn- red-rows
  "The image rows a red rule paints, down the middle of the panel."
  [pose]
...
(deftest a-rule-reads-its-intercept-in-the-space-it-was-given
  (testing ":in :drawing-area measures the intercept from the panel corner"
    ;; The intercept is a distance down the drawing area rather than a
    ;; data value, so the same number draws in the same place whatever
    ;; the data holds -- and 40 units down from a margin of 10 is row 50.
    (let [rule (fn [weights]
                 (-> {:height [1.0 5.0] :weight weights}
                     (pj/lay-point :height :weight)
                     (pj/lay-rule-h {:y-intercept 40 :in :drawing-area
                                     :color "#cc0000"})
                     (pj/options {:width 400 :height 300})))
          small (red-rows (rule [1.0 5.0]))
          large (red-rows (rule [300.0 400.0]))]
      (is (seq small) "no rule found")
      (is (= (first small) (first large))
          "a drawing-unit intercept moved with the data")))

  (testing "and a data-space intercept still reads the scale"
    (let [rule (fn [weights]
                 (-> {:height [1.0 5.0] :weight weights}
                     (pj/lay-point :height :weight)
                     (pj/lay-rule-h {:y-intercept 3.0 :color "#cc0000"})
                     (pj/options {:width 400 :height 300})))]
      (is (not= (first (red-rows (rule [1.0 5.0])))
                (first (red-rows (rule [1.0 500.0]))))))))

;; ---- Offsets on rules and bands ----

(defn- rule-layers-of [pose]
  (filterv #(#{:rule-h :rule-v :band-h :band-v} (:mark %))
           (:layers (first (:panels (pj/plan pose))))))

(deftest a-rule-carries-its-offset-to-the-plan
  (testing "the rule keeps the offset rather than dropping it"
    (is (= [20] (mapv :offset-y
                      (rule-layers-of (-> {:height [1 2] :weight [3 4]}
                                          (pj/lay-point :height :weight)
