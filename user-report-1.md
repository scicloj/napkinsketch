...
## Issue 1 — `pj/plot` is required for GFM render; no warning if omitted
...
**Summary.** If a pipeline ends at `pj/lay-line` (or any `lay-*`) without an explicit `pj/plot`, the expression returns a `scicloj.plotje.impl.sketch.Sketch` record. Clay treats it as a Kindly-kinded value — and in interactive HTML/Clay rendering this works fine — but **in `:format [:gfm]` the Sketch renders as nothing**: no SVG file is written, and the markdown has empty lines where the chart should be.

Appending `pj/plot` converts the Sketch into a hiccup `[:svg ...]` vector that Clay's GFM renderer picks up, writes as `image<N>.svg`, and references with a markdown `![]()` tag.

**Minimal repro.**

```clojure
;; Without pj/plot — renders blank in GFM
(-> {:x [1 2 3] :y [1 4 9]}
    (pj/lay-line :x :y))
;; => #scicloj.plotje.impl.sketch.Sketch{...}

;; With pj/plot — renders as an SVG image in GFM
(-> {:x [1 2 3] :y [1 4 9]}
    (pj/lay-line :x :y)
    pj/plot)
;; => [:svg ...] (hiccup vector)
...
**Expected.** Either (a) `pj/plot` should be implicit in `:format [:gfm]`, or (b) the README's "Quickstart" examples should make clear that bare `lay-*` expressions work only in interactive kinds, and a trailing `pj/plot` is mandatory for GFM/static export.

Note: plotje's own `notebooks/plotje_book/ranking.clj` omits `pj/plot` at the ends of its pipelines; the rendered `gfm/plotje_book.ranking.md` also has no `![]()` image references (though stale `image<N>.svg` files from an earlier render sit in the `_files/` directory). It looks like the GFM flow broke for this chapter at some point and wasn't caught.

---

## Issue 2 — `pj/lay-value-bar` rejects numeric `:x` at render time (reported; the user is already working on this)
...
>
> ```clojure
> (-> hourly-activity (pj/lay-value-bar :hour :messages {:x-type :categorical}))
> ```
>
> Documented in:
>
...
**Severity:** Medium — legitimate error but fires late (at `pj/plot`, not at `lay-value-bar`), and the remedy is to cast the column outside plotje.
...
(-> hourly-activity
    ...
    (pj/lay-value-bar :hour :messages)
    pj/plot)
```
...
## Issue 3 — `(pj/coord :flip)` draws the first row at the bottom, not the top
...
**Suggested change.** Either (a) provide an option like `(pj/coord :flip {:y-order :reverse})`, or (b) have flipped value-bar auto-reverse the categorical axis. (a) is safer and keeps current behavior explicit.

---

## Issue 4 — `pj/save-png` truncates the y-axis label to ~6 characters
...
>
> Logged in `CHANGELOG.md` under Known limitations → Marks (see "`pj/save-png`
...
(pj/save-png
 (-> {:x ["a" "b" "c"] :really-long-label [10 20 15]}
     (pj/lay-value-bar :x :really-long-label))
 "/tmp/repro-label.png")
...
**Severity:** Low — cosmetic, easily worked around with `(pj/options {:width N})`.
...
(pj/save-png
 (-> {:x [1 2 3 1 2 3]
     ...
     (pj/lay-line :x :y {:color :category}))
 "/tmp/repro-legend.png")
;; "tech.ml.dataset.dev" in the legend shows as "tech.ml.dataset."
```

**Workaround.** Pass a wider `:width` via `(pj/options {:width 900 :height 420})`.
...
- `pj/lay-line` with `{:color :channel}` — the multi-series coloring on a `tc/dataset` worked on the first try.
...
- `pj/svg-summary` — made it possible to assert chart structure (number of polygons, label set) from a REPL or test without rendering, a nice capability for regression-testing that the other obvious libraries lack.
...
3. **Issue 3** — consider an option like `(pj/coord :flip {:reverse-categorical true})` to spare every ranking-chart author from doing the ascending-sort dance.
